...
function crashController(req: Request, res: Response) {
  accessLog(req);
  if (!req.app.get("crash")) {
    req.app.set("crash", true);
  } else {
    req.app.set("crash", false);
  }
  res.status(200).json({
    status: "success",
    success: true,
    message: "Will start crashing!",
    data: {},
  });
}

export { commonHealth, commonStatus, crashController };
